...
#### Phase 6 이월 — UI 리프레시 + UX 개선 완료
- 전역 CSS 디자인 토큰 도입 (`--color-*`, `--radius-*`, `--shadow-*`, `--transition-*`) — PR #17
- AppLayout 헤더 / ReviewCard 카드 UI 스타일 개선 — PR #17
- BaseSelect 커스텀 드롭다운 컴포넌트 추가 (pill / input variant, 외부 클릭 닫힘, 애니메이션) — PR #17
- 리뷰 작성 폼 4스텝 멀티스텝 위자드 전환 (create 모드 전용, 스텝별 유효성 검증) — PR #17
- SubMetricsSection 펼치기/접기 슬라이드 애니메이션 추가 — PR #17
...
#### Phase 6 — UX 개선 2차 완료
- 리뷰 목록에 '내 기록 / 전체' 탭 분리 — PR #19
  - 기본 탭: 내 기록 (로그인 유저 리뷰만, 클라이언트 필터링)
  - 통계·필터·빈 상태 문구 탭 기준으로 독립 동작
- 리뷰 작성 폼 임시 저장 및 복원 — PR #19
  - 폼 변경·스텝 이동 시 localStorage 자동 저장
  - 재진입 시 바텀시트 다이얼로그로 '이어서 쓰기 / 새로 작성' 선택

#### Phase 6 — UX 개선 3차 완료
- 리뷰 카드에 방문일(YYYY.MM)·탈출시간(탈출 Xm) 표시 — PR #20
- 리뷰 목록 지점명·테마명 키워드 검색 추가 (클라이언트 필터링) — PR #20
...
- 리뷰 목록 토스화: hero 키커 제거·통계(총 N개/성공률) 한글화, 카드 배지(성공/실패)·메타·스포일러 mono→sans, 빈 상태/에러 messages 연결 — PR #55
- 리뷰 작성/수정 위자드 토스화: 컨셉 라벨(STEP/LOG·NEW/SECTION→N단계)·결과 토글(성공/실패) 한글화, 용어 "방"→"테마" 통일, 검증·에러 해요체+messages 연결 — PR #56
- 테마 검색 토스화: "방 검색"→"테마 검색"(네비/프로필 라벨 포함), 삭제 확인·리뷰 연결 경고 토스화, 토스트/에러 messages 연결 — PR #57
- 프로필 토스화: 로그아웃 다이얼로그 질문형 단순화, 통계 "세부 지표 평균"→"세부 평가 평균" 용어 통일 — PR #58
- 로그인/레이아웃 토스화: 인증 에러 영문 원문→친화 한국어 매핑, 가입 안내·로딩 문구 정리, 레이아웃 로그아웃 다이얼로그 단순화+aria-label — PR #59
- 공유 리뷰·작성자 프로필 토스화 + StatCard 한글 라벨/단위 mono→sans, 정책 페이지는 법률 본문 유지 — PR #60
- 메인 타이틀 "방탈출 기록장"→"방탈출 일지", 네비 "목록"→"홈"(상단+하단+상세 뒤로가기) — PR #60

#### Phase 10 이후 — 리뷰 상세 기능 개선
- 리뷰 상세 hero에 테마 포스터 표시(미등록·로드 실패 폴백 포함, 공유 화면 동일 적용) — PR #61
- 사진 라이트박스 열렸을 때 배경 스크롤 잠금 — PR #61

#### Phase 11 — v3 자연 톤 + '길' 체계 + 여정 타임라인 완료
- 디자인 토큰 v3: 클린 화이트 베이스 + 딥 틸(#11867a) 주조색 + 재미 등급 3색(꽃길 핑크/풀길 초록/흙길 갈색), 다크 hero→밝은 hero 토큰, 하드코딩 옛 파랑→brand 토큰 — PR #62
- 별점 전면 폐기: rating(1~5)을 5단계 '길' 사다리(흙길/흙풀길/풀길/풀꽃길/꽃길)로 입력(디딤돌)·표시(칩+도트)·필터(토글 칩)·정렬(꽃길순)·통계까지 통일, StarRating 삭제 — PR #62
- 내 기록 탭 여정 타임라인: 월 구분 헤더 + 여정 선(점 클릭 시 로그/월 스크롤, 30개 초과 시 월 점 적응형 줌, 전체 여정 경량 쿼리 fetchMyJourney) — PR #62
- 등급 칩 한글 세로 정렬 수정(.review-card__tags align-items: center) — PR #63
- v3 톤·'길' 체계 잔여 불일치 정리: 카카오 공유 카피(별점→길 라벨), 통계 '길 분포' 등급별 색, '평균 길' 한글값 깨짐(mono 옵트아웃), '평균 인원' accent 팔레트 정합, 상세 세부평가 막대 틸, 사진/약관 카피 해요체 — PR #64

#### Phase 11 후속 — 헤더·뱃지·여정·카드 다듬기
- 상세 헤더 액션을 고스트 아이콘 버튼으로 리디자인(테두리 박스·밑줄 링크 제거) — PR #65
- 반갈죽/삼갈죽 탈출 속도 자동 뱃지(60분 가정, escape-badge.ts + SpeedBadge) — PR #66
...
#### Phase 14 — 리뷰 작성 플로우 마감 품질 완료
- 작성·수정 중 하단 글로벌 탭바 숨김(AppLayout): 가운데 ＋FAB가 자기 자신을 가리키는 중복 + sticky 저장 버튼과의 겹침 해소, main 하단 여백·저장버튼 세이프에어리어 조정 — PR #77
- 한줄평 단일 input → 자동 확장 textarea: 100자 입력이 가로 스크롤로 잘려 보이지 않던 문제 해결 — PR #77
...
- [ ] SSOT 본문 갱신 — §3.1 폐기된 '별점' 표현, §3·§4 스포일러 필드 누락 (#77 범위 제외)
